...
export default function Home() {
  return (
    <main className="marketing-page">
      <nav className="marketing-nav"><Brand /><div><Link href="#product">Product</Link><Link href="#security">Security</Link><Link href="#foundation">Foundation</Link></div><div><Link className="nav-signin" href="/login">Sign in</Link><Link className="button button-primary button-small" href="/login?return_to=%2Fonboarding">Connect MT5</Link></div></nav>
      <section className="hero">
        <div className="hero-glow" />
        <div className="hero-copy"><div className="trust-chip"><span /> Read-only MT5 protection layer</div><h1>Protect your prop account <em>before</em> one bad trade ends the challenge.</h1><p>Monitor the rules that matter, understand the risk already open, and get warned before drawdown, consistency, or connection issues put the account at risk.</p><div className="hero-actions"><Link className="button button-primary" href="/login?return_to=%2Fonboarding">Connect your account <span>→</span></Link><Link className="button button-secondary" href="/dashboard">Explore the dashboard</Link></div><div className="hero-assurances"><span><b>✓</b> No trade execution</span><span><b>✓</b> No MT5 passwords</span><span><b>✓</b> Explainable calculations</span></div></div>
      ...
      <section className="cta-section"><div><p className="eyebrow">Build discipline into the challenge</p><h2>See the buffer before<br />you take the risk.</h2></div><div><p>Start with the secure account setup. Sourced rule versions affect an account only after independent approval and activation.</p><Link className="button button-light" href="/login?return_to=%2Fonboarding">Start account setup <span>→</span></Link></div></section>
