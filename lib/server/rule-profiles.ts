...
export async function ensureFundedNextRuleCatalog(database: AppDatabase): Promise<void> {
  ...
  for (const profile of fundedNextRuleProfiles) {
    ...
    statements.push(
      ...
      database.prepare("INSERT OR IGNORE INTO rule_versions (id, rule_set_id, version, effective_at, expires_at, verification_status, definition_json, content_hash, interpretation_notes, created_by_user_id, validated_by_user_id, reviewed_by_user_id, activated_by_user_id, approved_by_user_id, activated_at, superseded_at, rollback_of_version_id, created_at, updated_at) VALUES (?, ?, ?, ?, NULL, 'validated', ?, ?, ?, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, ?, ?)")
        .bind(profile.versionId, profile.ruleSetId, profile.version, FUNDEDNEXT_RULE_CAPTURED_AT, definitionJson, contentHash, notes, FUNDEDNEXT_RULE_CAPTURED_AT, FUNDEDNEXT_RULE_CAPTURED_AT),
